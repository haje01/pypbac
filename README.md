...
1. [릴리즈](https://github.com/haje01/pypbac/releases/)에서 최신의 압축 파일을 받고, 적당한 디렉토리에 풉니다.
...
- 이에 대처하기 위해 pypbac는 `사용자 홈폴더` 아래의 `.pypbac` 폴더에 데이터를 **캐쉬**로서 저장합니다.
...
- 일반적으로 캐쉬는 자동으로 갱신되기에 별도로 제거할 필요는 없습니다.
- 그러나, 최신 파일이 맞는지 의심스러운 경우 `불러오기 설정` 에서 `로컬 캐쉬 제거` 버튼을 눌러, 캐쉬를 명시적으로 제거할 수 있습니다.
